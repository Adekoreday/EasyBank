...
export default function Activate(props) {
    ...
    let activateAccountLoading = useSelector(state => state.account.activateAccountLoading);

    return (
        <div className="activate__account__content">

        {(modalStatus) && <Modal closeModal={CloseModal}>
         {  activateData!==null && (<div className="activate__modal">
         <div className="activate__modal__content">
         <div>{(activateAccounts!== null) ?
         ...
         <span className="failure_request">{activateAccounts.status} action on account failed</span>
         ) : '') : ''
         }</div>
          Kindly confirm the {activateData.status === 'active' ? <span className="deactivate">deactivate</span>: <span className="activate">activate</span>} 
          action on account: {activateData.accountNumber}</div>
         <div></div>
